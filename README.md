...
ورک‌فلو CI **یک APK یونیورسال امضاشده** می‌سازد و آپلود می‌کند:

| فایل | حجم تقریبی | کاربرد |
|---|---|---|
| `Hajmino-1.0.<versionCode>-build<run>-bazaar.apk` | ~۷.۶ MB | یونیورسال (همه ABIها)، بیلد کافه‌بازار (پرداخت Poolakey + جایگاه تبلیغات)، امضاشده |

> با کی‌ستور پایدار امضا می‌شود (سکرت‌های گیتهاب؛ کی‌ستور در ریپو نیست) و همهٔ
> آپدیت‌ها همان هویت را دارند. flavor `play` در CI ساخته **نمی‌شود** — فقط برای
> دیباگ محلی است (`./gradlew assemblePlayDebug`).
...
3. بعد از پایان، از صفحه ورک‌فلو روی **compressly-apks-…** کلیک کنید تا `Hajmino-1.0.<versionCode>-build<run>-bazaar.apk` و `SHA256SUMS.txt` دانلود شوند.
