...
ورک‌فلو CI چهار فایل APK می‌سازد و آپلود می‌کند:

| فایل | حجم تقریبی | کاربرد |
|---|---|---|
| `app-play-release-signed.apk` | ~۷.۶ MB | بیلد پلی (آفلاین کامل)، بهینه‌شده با R8، امضاشده برای تست |
| `app-bazaar-release-signed.apk` | ~۷.۶ MB | بیلد کافه‌بازار (جایگاه تبلیغات نمایان است)، امضاشده |
| `app-play-debug.apk` | ~۱۹ MB | نسخه دیباگ (لاگ بیشتر، نام پکیج با `.debug`) |
| `app-bazaar-debug.apk` | ~۱۹ MB | دیباگ بازار برای تست ظاهری اسلات تبلیغات |

> **توجه:** release ها با یک keystore موقت CI امضا شده‌اند تا قابل نصب باشند. برای انتشار در استورها باید با keystore شخصی خودتان امضا کنید (یا از Play App Signing استفاده کنید). نسخه‌های unsigned هم در همان artifact موجودند.
