# کامپرسلی (Compressly / Hajmino)

**عکس. ویدیو. موسیقی. کوچک‌تر.**

> **نکتهٔ نام‌گذاری (#۷):** نام مخزن در GitHub (`image-video-music-compress`)، نام برند
> فروشگاه («حاج‌مینو» / Hajmino)، نام بستهٔ اندروید (`ir.siliksama.hajmino`) و نام
> پروژهٔ Gradle (`Compressly`) همگی به یک اپلیکیشن اشاره دارند. این تفاوت عمدی نیست؛
> برای جستجو و ردیابی، این جدول را مبنای ارجاع قرار دهید:
> | جایگاه | مقدار |
> |---|---|
> | مخزن | `amirrezahadipoor/image-video-music-compress` |
> | برند فروشگاه | Hajmino |
> | ApplicationId / namespace | `ir.siliksama.hajmino` |
> | ریشهٔ Gradle | `rootProject.name = "Compressly"` |

کامپرسلی یک اپلیکیشن اندروید حرفه‌ای و کاملاً آفلاین برای فشرده‌سازی عکس، ویدیو و فایل‌های صوتی است. همه‌ی پردازش‌ها روی خود دستگاه انجام می‌شود؛ بدون اینترنت، بدون حساب کاربری، بدون آپلود.

![بنر کامپرسلی](art/banner.png)

---

## ویژگی‌های اصلی

- **۱۰۰٪ آفلاین.** فشرده‌سازی با API های بومی اندروید (MediaCodec، MediaExtractor، MediaMuxer، BitmapFactory، ExifInterface) انجام می‌شود. اپلیکیشن مجوز اینترنت ندارد (در بیلد پلی) و از اولین لحظه در حالت هواپیما قابل استفاده است.
- **سیستم درجه‌بندی یکپارچه.** عکس، ویدیو و صدا همگی از یک نوار ۴ مرحله‌ای استفاده می‌کنند: *حداکثر کیفیت*، *متعادل*، *فشرده‌سازی بالا*، *حداکثر فشرده‌سازی* — با نمایش درصد کاهش حجم تقریبی قبل از شروع + حالت پیشرفته برای کنترل دقیق.
- **حالت هوشمند (پیش‌فرض).** برنامه به‌صورت خودکار بهترین تعادل کیفیت/حجم را پیدا می‌کند: تا حد ممکن فشرده می‌کند و همیشه بیش از ۷۰٪ کیفیت را حفظ می‌کند (کیفیت ۸۵→۶۵ تطبیقی برای عکس، فرمول بیت‌ریت کیفیت‌محور و محدودیت ۱۹۲۰p برای ویدیو، ۱۹۲kbps برای صدا). هر تنظیم دستی کاربر، کنترل را به خودش برمی‌گرداند.
- **موتورهای کیفیت‌محور.** رونویسی ویدیو با سخت‌افزار (MediaCodec surface pipeline با پشتیبانی H.264/H.265 و فالبک نرم‌افزاری)، رمزگشایی نمونه‌ای عکس با مدیریت EXIF، و انکودر سخت‌افزاری AAC به‌همراه انکودر MP3 (LAME) داخل‌برنامه‌ای.
- **پردازش گروهی.** چند فایل با یک پروفایل؛ پیشرفت هر فایل و کل گروه؛ لغو تک‌فایل یا کل کار.
- **ادامه در پس‌زمینه.** کارهای طولانی در سرویس پیش‌زمینه اجرا می‌شوند و اعلان پیشرفت با دکمه‌های توقف/ادامه/لغو دارند.
- **اعداد واقعی.** حجم اصلی، حجم تقریبی خروجی، حجم واقعی و درصد کاهش — قبل، حین و بعد از فشرده‌سازی.
- **بدون تبلیغات و بدون ثبت‌نام** (در بیلد پیش‌فرض/پلی).

## بخش‌ها

### عکس
- ورودی: JPEG، PNG، WebP، HEIC/HEIF (خواندن؛ اندروید ۹ به بالا)
- خروجی: JPEG، WebP، PNG یا حفظ فرمت اصلی
- اسلایدر کیفیت + پری‌ست‌های تغییر اندازه (۲۵۶۰/۱۹۲۰/۱۲۸۰/۱۰۲۴/دلخواه با حفظ نسبت)
- حفظ متادیتا EXIF به‌صورت پیش‌فرض یا حذف برای حریم خصوصی
- پیش‌نمایش زنده «قبل/بعد» با اسلایدر کشیدنی و تخمین حجم لحظه‌ای

### ویدیو
- ورودی: MP4، MOV، MKV، WebM
- خروجی: MP4 (H.264 یا H.265 در صورت پشتیبانی سخت‌افزار؛ توضیح تفاوت‌ها داخل اپ)
- رزولوشن (اصلی/۱۰۸۰p/۷۲۰p/۴۸۰p/دلخواه)، بیت‌ریت، نرخ فریم
- صدای ویدیو: حفظ، فشرده‌سازی جدا، یا حذف
- برش (نقطه شروع/پایان) قبل از فشرده‌سازی
- سرویس پیش‌زمینه با اعلان؛ پشتیبانی از توقف و لغو

### صدا / موسیقی
- ورودی: MP3، WAV، FLAC، AAC، OGG، M4A
- خروجی: AAC (M4A) سخت‌افزاری یا MP3 با LAME داخلی — CBR یا VBR، ۶۴ تا ۳۲۰ کیلوبیت
- حفظ برچسب‌های ID3/MP4 (عنوان، خواننده، آلبوم، کاور) با گزینه حذف
- پردازش گروهی + نمایش موج‌نگار سبک

### امکانات دیگر
- تاریخچه با فیلتر بخش‌بندی‌شده (همه / عکس / ویدیو / صدا) و دسترسی سریع به فایل‌های فشرده
- اشتراک‌گذاری و ذخیره در پوشه‌های استاندارد (Pictures/Movies/Music/Hajmino)
- تم روشن/تیره با انتخاب کاربر (پیروی از سیستم به‌صورت پیش‌فرض)
- **زبان پیش‌فرض فارسی** + انتخاب زبان انگلیسی از تنظیمات
- افکت‌های صوتی سبک (سنتزشده داخل اپ، بدون فایل اضافه) با امکان خاموش‌کردن
- لودینگ‌های انیمیشنی با نمایش زمان دقیق (گذشت و زمان باقی‌مانده)
- بازیابی پس از کرش: اگر برنامه به‌طور غیرمنتظره بسته شود، در اجرای بعدی پیام شفاف نشان داده می‌شود و کارهای ناتمام «ناقص» علامت می‌خورند

## زبان‌ها

- پیش‌فرض: **فارسی** (با چیدمان راست‌به‌چپ) — حتی اگر زبان دستگاه انگلیسی باشد، برنامه فارسی باز می‌شود
- انگلیسی: از تنظیمات → «زبان برنامه» می‌توانید به English تغییر دهید (تغییر فوری)

## نسخه‌های خروجی (Flavors)

| فلِیور | کاربرد | اینترنت | تبلیغات |
|---|---|---|---|
| `play` | بیلد آفلاین (سایدلود) — **بدون فروشگاه و بدون پرداخت** | ندارد (کاملاً آفلاین) | ندارد |
| `bazaar` | کافه‌بازار | فقط برای SDK تبلیغات (Adivery) | بنر Adivery |

- **تبلیغات (Adivery):** سه اسلات در صفحه اصلی، تاریخچه و نتیجه (`AdSlot`). در بیلد `bazaar` بنر واقعی از SDK **Adivery** بارگذاری می‌شود (کلاس `AdiveryAdsProvider` در سورس‌ست bazaar). بیلد `play` هیچ مجوز اینترنت و هیچ SDK تبلیغاتی ندارد.
- **پرداخت:** خرید پریمیوم (حذف تبلیغات) فقط در بیلد `bazaar` از طریق کافه‌بازار (Poolakey) فعال است؛ صفحه‌ی پریمیوم در هر دو بیلد در دسترس است و در بیلد `play` پیام شفاف نمایش می‌دهد.
- **کافه‌بازار:** بیلد bazaar هیچ وابستگی به Google Play Services ندارد و برای انتشار در کافه‌بازار آماده است.`
- **کافه‌بازار:** بیلد bazaar هیچ وابستگی به Google Play Services ندارد و برای انتشار در کافه‌بازار آماده است.

## تست‌ها

یک مجموعه تست واحد JVM در CI قبل از ساخت اجرا می‌شود (`:app:testPlayDebugUnitTest` و `:app:testBazaarDebugUnitTest`):

- **FormatsTest** — قالب‌بندی حجم/درصد/مدت
- **SizeEstimatorTest** — یکنواختی تخمین کیفیت عکس، محدوده تخمین ویدیو، فرمول بیت‌ریت هوشمند، تطابق بیت‌ریت صدا
- **PresetDefaultsTest** — هوشمند پیش‌فرض است، همه پریست‌ها برای هر سه نوع وجود دارند، بازه‌های کاهش معتبر
- **JobControlTest** — سمانتیک توقف/لغو
- **Mp3WriterTest** — تست **واقعی** انکودر: PCM واقعی از LAME جاوا عبور داده می‌شود و خروجی MP3 (همگام‌سازی فریم MPEG، تفاوت CBR/VBR، استقلال از اندازه‌ی تکه‌ها) بررسی می‌شود

این حلقه در عمل باگ‌های واقعی پیدا کرد: تخمین ویدیو ۱٬۰۰۰٬۰۰۰ برابر و تخمین صدا ۱٬۰۰۰ برابر بزرگ‌تر از حد واقعی بود (هر دو فیکس شد).

## ساخت (Build)

```bash
./gradlew :app:assemblePlayRelease      # خروجی release پلی
./gradlew :app:assembleBazaarDebug      # خروجی debug بازار (مناسب تست)
./gradlew :app:assemblePlayDebug        # دباک معمولی
```

برای release نهایی باید keystore خود را در `signingConfigs` تنظیم کنید.

### خروجی‌های آماده (artifact)

ورک‌فلو CI دو **APK یونیورسال امضاشده** می‌سازد و آپلود می‌کند:

| فایل | حجم تقریبی | کاربرد |
|---|---|---|
| `Compressly-<نسخه>-play.apk` | ~۷.۶ MB | یونیورسال (همه ABI ها)، بیلد پلی (آفلاین کامل)، امضاشده |
| `Compressly-<نسخه>-bazaar.apk` | ~۷.۶ MB | یونیورسال، بیلد کافه‌بازار (جایگاه تبلیغات)، امضاشده |

> هر دو با کی‌استور پایدار امضا می‌شوند (سکرت‌های گیتهاب) و همه آپدیت‌ها همان هویت را دارند.

## CI — خروجی خودکار APK از گیت‌هاب

ورک‌فلو `.github/workflows/build.yml` به‌صورت خودکار با هر push روی `main` (یا به‌صورت دستی از تب Actions) **فقط APK امضاشده‌ی بازار** را می‌سازد و آپلود می‌کند:

1. به تب **Actions** ریپو بروید.
2. ورک‌فلو **Build & Test Bazaar APK** را انتخاب و روی **Run workflow** بزنید.
3. بعد از پایان، از صفحه ورک‌فلو روی **compressly-apks-…** کلیک کنید تا `Hajmino-1.0.0-…-bazaar.apk` و `SHA256SUMS.txt` دانلود شوند.

> هیچ APK مخصوص گوگل‌پلی (flavor «play») در CI ساخته یا آپلود نمی‌شود؛ flavor «play» فقط برای دیباگ محلی است.

جدای از ساخت APK، یک جاب **Emulator smoke test** هم روی هر push اجرا می‌شود (API 35، با KVM): یک کلیپ H.264 کوچک روی خود امولاتور تولید و از کل خط لوله‌ی واقعی MediaCodec عبور داده می‌شود — همان تستی که تست‌های JVM نمی‌توانند پوشش دهند. این جاب عمداً مستقل از ساخت APK است تا امولاتور لَنگ نتواند تحویل APK را متوقف کند.

## حمایت مالی (اختیاری)

دکمه‌ی قلب تپنده در بالای داشبورد به صفحه‌ی «حمایت مالی» می‌رود. این صفحه دو کانال دارد:

- **کافه‌بازار:** دکمه صفحه‌ی برنامه را در کافه‌بازار باز می‌کند (کانال رسمی «حمایت مالی» فروشگاه).
- **کارت به کارت:** فقط وقتی نمایش داده می‌شود که شماره کارت در زمان بیلد تنظیم شده باشد (این پروژه هیچ شماره کارتی در ریپو ندارد):

```bash
# بیلد بازار با کارت (اختیاری)
SUPPORT_CARD_NUMBER="6104-…" SUPPORT_CARD_HOLDER="نام صاحب کارت" ./gradlew :app:assembleBazaarRelease
```

یا در `~/.gradle/gradle.properties`: `supportCardNumber=…` و `supportCardHolder=…`. اگر تنظیم نشود، بخش کارت به‌کلی مخفی می‌ماند و فقط حمایت بازار نمایش داده می‌شود؛ هیچ کارت جعلی‌ای نمایش داده نمی‌شود.

## کاهش حجم برنامه

- R8 + ShrinkResources در release
- `resConfigs("fa","en")` — فقط دو زبان نگه داشته می‌شود
- تصاویر برنامه به WebP فشرده شدند (۲.۸MB → ~۲۱۲KB)
- بدون فایل‌های باینری اضافه؛ صداها به‌صورت سنتزشده در حافظه تولید می‌شوند

## ساختار فنی

```
ui/        کامپوز اسکرین‌ها، ViewModel ها، تم و کامپوننت‌های مشترک
core/
  engine/  موتورهای عکس | ویدیو | صدا، تخمین‌گر حجم، بازرس رسانه
  service/ هماهنگ‌کننده‌ی کارها + سرویس پیش‌زمینه + اعلان‌ها
  data/    تاریخچه (Room)، تنظیمات (DataStore)، خروجی MediaStore
  ads/     لایه تبلیغات (Noop پیش‌فرض / Adivery در bazaar)
  util/    قالب‌بندی، صداها، زبان، بازیابی کرش
```

## محدودیت‌های صادقانه

- **عکس HEIC/HEIF** به اندروید ۹+ (API 28) نیاز دارد؛ در نسخه‌های پایین‌تر پیام شفاف نمایش داده می‌شود.
- **انکودر MP3** نسخه خالص جاوا از LAME است (jump3r)؛ کاملاً آفلاین ولی از AAC سخت‌افزاری کندتر است. برای بیشتر کارها AAC/M4A گزینه بهتر است.
- **خروجی H.265** به وجود انکودر HEVC در دستگاه بستگی دارد؛ در غیر این صورت با پیام واضح به H.264 برمی‌گردد.
- در اندروید ۱۴+ سرویس‌های `dataSync` محدودیت زمانی (۶ ساعت) دارند — برای فایل‌های معمولی عملاً بی‌اثر است.
- نوشتن متادیتا بهترین تلاش است؛ فایل خروجی همیشه سالم تحویل داده می‌شود حتی اگر برچسب‌نویسی ناموفق باشد.

## مجوزها

قبل از انتشار، فایل `LICENSE` دلخواه خود را اضافه کنید. اجزای شخص ثالث: jump3r (تبار LGPL/GPL — برای توزیع بررسی کنید)، jaudiotagger (LGPL).
