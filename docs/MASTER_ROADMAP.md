...
| **BUG-6** | `SizeEstimator.kt` | `estimateAudio` یک فرمول دستی داشت و گزارش اول «واحد اشتباه» را نتیجه گرفته بود؛ بررسی واحدها نشان داد `durationMs * bitrateKbps / 8` درست است (ms × kbps / 8 = bytes). کد امروز اصلاً همان فرمول را ندارد: `SizeEstimator.estimateAudio` به `AudioPlanner.targetBitrateKbps` + `AudioPlanner.estimatedBytes(bitrate, ms, vbr)` واگذار شده، و `vbr` از `bitrateMode` می‌آید — که از ۱۴۰۴-۰۶ واقعاً قابل تنظیم است (چیپ VBR/CBR اضافه شد)، پس `factor = 0.92` دیگر همیشه ۱.۰ نیست. | ✅ درست‌سازی شد (فرمول به AudioPlanner منتقل شد) |
