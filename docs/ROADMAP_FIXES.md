...
| P5 | عکس‌های **فولدری** با «حذف اصلی + بازگشت به همان فولدر» هنوز اصلی‌شان پاک نمی‌شود | سه ریشهٔ جدا پیدا شد و هر سه بسته شد: **(۱)** دروازهٔ اجازه دور زده می‌شد (P7)؛ **(۲)** عکس‌های HEIC/PNG که به JPEG تبدیل می‌شدن از مسیر in-place حذف می‌شدند (`mimeAllowsInPlaceReplace` تطابق دقیق MIME می‌خواست) → فایل جدید publish + حذف اصلی که اندروید رد می‌کند؛ حالا `OutputStore.retypeMediaRow` اول MIME/پسوند خودِ رکورد را اصلاح می‌کند و نوشتن در همان رکورد انجام می‌شود، پس اصلاً حذفی لازم نیست؛ **(۳)** `OutputStore.delete` برای رکورد `content://media` اول `DocumentFile.fromSingleUri` را صدا می‌زد (ارائه‌دهندهٔ اسناد، نه مالک ردیف)؛ حالا MediaStore اول می‌آید و نتیجهٔ واقعی برمی‌گردد. **اثبات روی دستگاه باقی است**: یک پوشهٔ mix (چند HEIC + چند JPEG) با «جایگزینی اصلی» روشن | `[~]` |
| P6 | دکمهٔ بک/HOME گوشی در چند جا روی نوشته‌ها می‌افتد | IME و نوار ناوبری در `bottomBar` چسبان و اورلی‌ها اعمال نشده | `[x]` |
| P7 | هر دو مسیر «ادامه با وجود کمبود فضا» و بازگشت از پنجرهٔ اجازهٔ نوتیفیکیشن مستقیماً `startCompression()`/`forceCompress()` را صدا می‌زدند و **دروازهٔ اجازهٔ MediaStore را رد می‌کردند** → نوشتن رد می‌شد، فایل دوم ساخته می‌شد، حذف اصلی رد می‌شد: همان «عکس‌ها پاک نشدند» | `[x]` — یک ماشین‌حالت واحد (`requestCompression` → `launchConsent` → `finishAfterConsent`)؛ هیچ ورودی جز دروازه شروع نمی‌کند. grantِ نوشتن و حذف هم زنجیر می‌شوند (دو پنجره، چون `createWriteRequest` اجازهٔ delete نمی‌دهد) |
| P8 | برآورد فضا `firstFile × N` بود؛ در یک دستهٔ بزرگ **هشدار کمبود فضای جعلی** می‌داد و همان دیالوگ، دروازهٔ اجازه را هم دور می‌زد | `[x]` — قاعده در `JobTotals.estimateBatchBytes` جمع شد و هر دو مصرف‌کننده (UI و گیت فضا) همان را صدا می‌زنند؛ تست: `ReplaceOriginalPolicyTest` |
